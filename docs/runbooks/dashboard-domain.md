...
Terraform registers `antelab.eu` with Scaleway for one year, enables annual
auto-renewal, and creates an A record for `apollo.antelab.eu` pointing to the
...
1. Search for `antelab.eu` in Scaleway **Domains and DNS**. If unavailable,
